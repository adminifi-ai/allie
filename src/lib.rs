...
fn resolve_model_credentials() -> Option<&'static ModelProviderPreset> {
...
fn model_provider_preset_env_names() -> String {
    MODEL_PROVIDER_PRESETS
        .iter()
...
impl ModelPolicy {
    ...
    /// Build the model policy `allie init` scaffolds: enabled with a
    /// populated `provider_allowlist` when a resolvable API key is found in
    /// the process environment, otherwise left at the (disabled) default so
    /// `allie doctor` can explain why. Never returns `enabled: true` with an
    /// empty `provider_allowlist` — that combination fails the manifest's
