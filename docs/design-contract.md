...
| `https://cdn.jsdelivr.net/gh/misty-step/aesthetic@9bbe0f9/aesthetic.css` | Exact requested commit CSS path. | observed | high | change | `curl -I` returned HTTP 404. The npm package `@misty-step/aesthetic` also returned registry 404, so Allie records an explicit local-subset adoption decision. |
| `src/report.rs` | WCAG compliance report is the first representative adopted surface. | observed | high | keep | This surface carries the complete ledger, status proof strip, screenshots, and review evidence, making it the best first adoption point. |
| `VISION.md` and `README.md` | Evidence clarity and no legal-compliance promise are product invariants. | observed | high | keep | Design must not obscure status, confidence, residual review, or replay provenance. |
