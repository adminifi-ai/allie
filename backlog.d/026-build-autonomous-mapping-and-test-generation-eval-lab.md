...
Board of record: Habitat (this epic = AL-002). Plan: `docs/plans/032-mega-groom-execution.html`.
- Prerequisite gap: this epic measures whatever mapper exists, but the mapper cannot see modern apps — `src/discovery/live.rs` is a raw-TCP, `http://`-only, static-HTML crawler (live.rs:91-99, 323-354, 372-374), and `src/discovery/steps.rs:43` hard-gates step generation to `local_fixture`. Without new capability, children 4-6 would grade permanent ~zero autonomous recall on every real target. New cards **AL-114** (browser-worker discovery mode: Playwright crawl, SPA/HTTPS/auth via existing AuthFlow/storageState, `source: "browser-worker-crawl"`, packet schema unchanged) and **AL-115** (DOM-driven step generation for live targets, replay-gated via the existing promotion machinery) are sequenced ahead of the recall/replay graders.
- The raw-TCP crawler is a deletion candidate (~414 LOC + dedicated test TCP server) once AL-114 proves recall parity on this epic's corpus — proposal, not applied.
