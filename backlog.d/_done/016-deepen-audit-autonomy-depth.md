...
- 2026-06-30: Agentic review now fans out across the evidence packet's captured
  surfaces. Rust sends `coverage.state_metadata` as review surfaces, the worker
  captures per-surface evidence with retry, model calls retry transient failures
  inside the same budget, and per-obligation aggregation applies fail precedence
  across surfaces. `npm run agentic:smoke` proves the fake OpenRouter home +
  settings fanout, retry, video payloads, and observe-act-rejudge loop offline;
