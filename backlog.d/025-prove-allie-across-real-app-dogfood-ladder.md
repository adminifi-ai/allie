...
- 2026-07-01: Vanity static/public slice delivered in `docs/dogfood/vanity/025-vanity-static-dogfood.md` (child 4), with the pinned manifest `docs/dogfood/vanity/025-vanity-home.yml`. Clean checkout `68a15d0`, byte-identical `git status` before/after (verified twice), 1 served surface, 0 infrastructure failures, release blocked on 1 deterministic `1.4.3` contrast criterion; the two agentic findings (`1.4.11`, `2.2.2`) stayed advisory. Deterministic gate proven byte-stable across two runs while the agentic layer drifted.
- 2026-07-01: Cross-target summary delivered in `docs/dogfood/025-vanity-vs-olympus-cross-target.md` (child 8). Files mapping-precision/recall and auth-friction misses into epic 026 and report-efficacy misses (model-off `needs_review` wall, silent-gap roll-up, unreconciled review counts, non-stable headline counts, non-self-describing packet dir) into epic 027; both epics updated in place.
- Ladder coverage so far: static/public (Vanity) and operations/control-plane (Olympus). Remaining classes: authenticated consumer (Linejam refresh), work-management (Habitat).
